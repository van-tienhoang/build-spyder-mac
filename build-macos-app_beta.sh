...
"${APPDIR}"/Contents/MacOS/python -m pip install pip~=18.0 wheel
"${APPDIR}"/Contents/MacOS/python -m pip install --pre -U spyder
...
exec "${DIR}"/PythonApp -m spyder.app.start "$@"
EOF
...
VERSION=$("${PYTHON}" -m pip show pip | grep -E '^Version:' |
          cut -d " " -f 2)
